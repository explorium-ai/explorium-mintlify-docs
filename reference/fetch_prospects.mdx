...
| job\_department | Filter prospects by their job department. Categories: \[customer service, design, education, engineering, finance, general, health, sales, ...\] | Use autocomplete to browse and select departments. Works best when combined with job\_level to cover title variations across organizations. Uses multiple `values`. Matches selected categories against a normalized list. |
