...
<AccordionGroup>
  ...
  <Accordion title="Prospect Filters">
    ...
    | job_level                   | Filter prospects by their job level. Categories: [director, manager, vp, partner, cxo, non-managerial, senior, entry, training, unpaid]. In the response, two additional fields are included — `job_level_array` and `job_level_main`, which represent the extended hierarchical taxonomy for this attribute. Example hierarchy: cxo → c-suite → executive, manager → senior manager → leadership, junior → entry-level.                               | Use autocomplete to search and select job levels. You can choose one or more values. The response fields (`job_level_array`, `job_level_main`) provide enriched hierarchical context. Examples of new levels: manager, president, senior manager, owner, advisor, freelancer, junior, director, c-suite, board member, senior non-managerial, non-managerial, partner, vice president, founder.                                                                                                                                                                                                         |
    | job_department              | Filter prospects by their job department. Categories: [customer service, design, education, engineering, finance, sales, etc.]. In the response, two additional fields are included — `job_department_array` and `job_department_main`, which represent the extended hierarchical taxonomy for department classification. Example hierarchy: engineering → R&D → technology, sales → customer success → commercial, finance → accounting → operations. | Use autocomplete to browse and select departments. Works best when combined with job_level to ensure title coverage across organizations. The response fields (`job_department_array`, `job_department_main`) provide enriched hierarchical context. Examples of new departments: retail, engineering, customer success, administration, education, security, healthcare, public service, partnerships, creative, strategy, real estate, procurement, IT, data, c-suite, manufacturing, support, logistics, product, sales, design, marketing, finance, R&D, trade, human resources, legal, operations. |
    | related_job_titles          | Expand filtering results to include related job titles.                                                                                                                                                                                                                                                                                                                                                                                                | When set to `true`, the system broadens the search to include job titles that are semantically or hierarchically related to the provided title(s). Default is `false`. Useful for retrieving a wider range of relevant roles.                                                                                                                                                                                                                                                                                                                                                                           |
