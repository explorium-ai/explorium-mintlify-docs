...
`fetch-businesses` and `fetch-prospects` return full records, and each returned result consumes
**1 credit** — **2 credits** when the `business_intent_topics` intent filter is part of the query.
Fetch once with tight filters, then use the matching enrich tool to add deeper signals
(contacts, tech stack, workforce, and the rest) only for the IDs you keep.
...
3. **Unbounded fetch loops burn credits.** Every result returned by `fetch-businesses` / `fetch-prospects` consumes 1 credit (2 with the `business_intent_topics` intent filter), so a model that re-fetches with varied filters multiplies cost fast. Fetch once with tight filters and reuse the results; use the free statistics tools to size an audience before fetching.
