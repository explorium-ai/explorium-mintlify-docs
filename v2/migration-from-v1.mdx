...
<Steps>
  ...
  <Step title="Handle removed response fields">
    Stop reading `entity_id`, `display_name` on business LinkedIn posts, the nested `emails` / `phones` contact fields, and the address signals on `company_ratings_by_employees`.
  </Step>
...
| `/v1/businesses/research/bulk_enrich` | `/v2/businesses/research/enrich` | Async variant follows post-beta |
| `/v1/prospects/research/bulk_enrich` | `/v2/prospects/research/enrich` | Async variant follows post-beta |
...
| Events, enrollments, webhooks, credits | `/v2/...` equivalents | Path change only |
...
- [ ] Nested `emails` / `phones` contact fields no longer read
