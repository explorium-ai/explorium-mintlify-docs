...
<Warning>
  **Upcoming (before GA):** this event is being renamed to `executive_joined_company` across every v2 surface — fetch filters, enrollment `event_types`, webhook simulation, and the `event_name` in delivered payloads. See [Coming before GA](/v2/whats-new#coming-before-ga).
</Warning>
