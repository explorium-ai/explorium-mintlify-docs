...
| `emails` | All email addresses found, each with an `address` and a `type` (e.g. `current_professional`, `personal`). **Removed before GA** — see [Coming before GA](/v2/whats-new#coming-before-ga); use `professional_email`. |
