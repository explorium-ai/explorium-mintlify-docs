...
Research runs on **two entity types**, each with its own sync and async endpoints:
...
Inside a `prompt_template`, reference any of the fields below with `{{ record['field_name'] }}`. When you pass a `query` instead, these same fields are made available to the prompt generator as the entity's context. The available fields depend on the endpoint you call.
