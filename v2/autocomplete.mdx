...
The **Autocomplete** endpoint provides **real-time suggestions** for filter fields, helping users quickly find relevant categories, locations, industries, and job attributes based on partial text input. This is useful for creating dynamic search fields and enhancing user experience in filtering interfaces.

<Note>
  **One endpoint in v2.** Autocomplete is consolidated into a single `/v2/autocomplete` endpoint serving both business and prospect fields. The v1 split of `/v1/businesses/autocomplete` and `/v1/prospects/autocomplete` is gone — point every autocomplete call at `/v2/autocomplete` and select what you want with the `field` parameter.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Example Request (cURL)">
    ...
    ```shell Bash
    curl -X GET \
      "https://api.explorium.ai/v2/autocomplete?field=country&query=unit" \
      -H "API_KEY: your_api_key_here"
    ```
   Example with business_intent_topics field. 
    ```shell Bash
    curl -X GET \
      'https://api.explorium.ai/v2/autocomplete?query=cloude&semantic_search=true&field=business_intent_topics' \
      -H "API_KEY: your_api_key_here"
    ```
