...
<Warning>
  **The API playground on these pages points at the dev environment**, because v2 is not yet on production. Requests you send from the playground hit `https://platform-dev-us-app.explorium.ai/api/partner-service` and need a dev API key. The server must be switched to `https://api.explorium.ai` before v2 docs go live.
</Warning>
