...
Every `.../job` request also accepts an optional `notifications` object for email updates instead of polling:
...
The `on` list picks which terminal states trigger an email — `success`, `failure`, or both.
