...
<Note>
  The spec types `status` as a free-form string and does not enumerate its values, so the sample above keys off `results.download_url` and `error` rather than matching on specific status text. That is the more robust pattern regardless.
</Note>
